...
class EurekaServerTest {
	...
	@Autowired
	EurekaClientConfig configuration;

	@Autowired
	PeerEurekaNodes peers;
	...
	void standaloneRegistryAcceptsAndRemovesAccountRegistration() throws Exception {
		...
		String registration = """
				...
				""";
		HttpResponse<String> registered = client.send(
				request("/eureka/apps/ACCOUNT").header("Content-Type", "application/json")
						.POST(HttpRequest.BodyPublishers.ofString(registration)).build(),
				HttpResponse.BodyHandlers.ofString());
		try {
			assertThat(registered.statusCode()).isEqualTo(204);
			HttpResponse<String> found = client.send(
					request("/eureka/apps/ACCOUNT/account-test").header("Accept", "application/json").GET().build(),
					HttpResponse.BodyHandlers.ofString());
			assertThat(found.statusCode()).isEqualTo(200);
			assertThat(found.body()).contains("account-test");
		}
		finally {
			client.send(request("/eureka/apps/ACCOUNT/account-test").DELETE().build(),
