...
@SpringBootTest(properties = { "spring.datasource.url=jdbc:h2:mem:account;MODE=PostgreSQL;DB_CLOSE_DELAY=-1",
...
class AuthenticationIntegrationTest {
	...
	void duplicateEmailIsRejectedAfterNormalization() {
		register();
		assertError(() -> auth.register(new RegisterRequest(" PERSON@EXAMPLE.TEST ", PASSWORD)),
	...
	void verificationExpiresAtExactBoundary() {
		...
		assertThat(users.findById(result.userId()).orElseThrow().getStatus())
	...
	void refreshRotatesTokenAndRejectsReuse() {
		...
		assertThat(refreshTokens.findByTokenHash(tokens.hash(first.refreshToken())).orElseThrow().getRevokedAt())
				.isEqualTo(NOW);
		assertThat(refreshTokens.findByTokenHash(tokens.hash(second.refreshToken())).orElseThrow().getRevokedAt())
				.isNull();
	...
	void forgotPasswordHasSamePublicResponseForExistingAndMissingEmail() throws Exception {
		activate();
		String existing = mvc
				.perform(post("/v1/public/auth/forgot-password").contentType("application/json")
	...
	void resetChangesPasswordConsumesAllResetTokensAndRevokesAllSessions() {
		...
		assertThat(encoder.matches(NEW_PASSWORD, credentials.findByUserId(id).orElseThrow().getPasswordHash()))
				.isTrue();
		...
		assertError(() -> passwords.resetPassword(new ResetPasswordRequest(firstReset, PASSWORD)),
				INVALID_PASSWORD_RESET_TOKEN);
		assertError(() -> passwords.resetPassword(new ResetPasswordRequest(secondReset, PASSWORD)),
				INVALID_PASSWORD_RESET_TOKEN);
	...
	void resetRejectsInvalidAndExpiredTokensWithoutChangingPassword() {
		...
		assertError(() -> passwords.resetPassword(new ResetPasswordRequest("missing", NEW_PASSWORD)),
				INVALID_PASSWORD_RESET_TOKEN);
		clock.now = NOW.plus(Duration.ofMinutes(30));
		assertError(() -> passwords.resetPassword(new ResetPasswordRequest(raw, NEW_PASSWORD)),
	...
	void passwordChangeVerifiesCurrentPasswordAndRevokesSessionsAndResetLinks() {
		...
		assertThat(encoder.matches(NEW_PASSWORD, credentials.findByUserId(id).orElseThrow().getPasswordHash()))
				.isTrue();
		assertError(() -> auth.refresh(new RefreshTokenRequest(refresh)), INVALID_REFRESH_TOKEN);
		assertError(() -> passwords.resetPassword(new ResetPasswordRequest(reset, PASSWORD)),
				INVALID_PASSWORD_RESET_TOKEN);
	}
	...
	@ParameterizedTest
	@ValueSource(strings = { "{\"email\":\"bad\",\"password\":\"valid-password\"}",
			"{\"email\":\"person@example.test\",\"password\":\"short\"}", "{\"email\":\"person@example.test\"}",
	...
	void registrationValidationDoesNotLeakInput(String request) throws Exception {
		mvc.perform(post("/v1/public/auth/register").contentType("application/json").content(request))
				.andExpect(status().isBadRequest()).andExpect(jsonPath("$.code").value("INVALID_REQUEST")).andExpect(
						content().string(org.hamcrest.Matchers.not(org.hamcrest.Matchers.containsString("password"))));
	...
	void publicRoutesReachValidationAndPrivateRoutesRequireBearerToken() throws Exception {
		for (String endpoint : List.of("register", "verify-email", "login", "refresh", "forgot-password",
				"reset-password")) {
	...
	void protectedPasswordChangeUsesJwtSubject() throws Exception {
		UUID id = activate();
		AuthTokenResponse session = login();
		mvc.perform(post("/v1/auth/change-password").header("Authorization", "Bearer " + session.accessToken())
				.contentType("application/json")
				.content("{\"currentPassword\":\"initial-password\",\"newPassword\":\"updated-password\",\"userId\":\""
						+ UUID.randomUUID() + "\"}"))
	...
	void jwtSignatureExpiryAndSubjectAreCheckedAndUnlistedRoutesAreDenied() throws Exception {
		activate();
		AuthTokenResponse session = login();
		mvc.perform(post("/v1/auth/logout").header("Authorization", "Bearer invalid").contentType("application/json")
				.content("{\"refreshToken\":\"missing\"}")).andExpect(status().isUnauthorized());
		mvc.perform(post("/unlisted").header("Authorization", "Bearer " + session.accessToken()))
				.andExpect(status().isForbidden());
		String malformedIdentity = jwtEncoder.encode(JwtEncoderParameters
				.from(JwsHeader.with(MacAlgorithm.HS256).build(), JwtClaimsSet.builder().subject("not-a-uuid")
						.claim("email", EMAIL).issuedAt(NOW).expiresAt(NOW.plusSeconds(900)).build()))
				.getTokenValue();
	...
	void httpFlowCoversRegistrationActivationRotationResetChangeAndLogout() throws Exception {
		...
		mvc.perform(post("/v1/public/auth/verify-email").contentType("application/json")
				.content("{\"token\":\"" + delivery.last().token() + "\"}")).andExpect(status().isOk());
		String loginBody = mvc
				.perform(post("/v1/public/auth/login").contentType("application/json")
						.content("{\"email\":\"person@example.test\",\"password\":\"initial-password\"}"))
				.andExpect(status().isOk()).andReturn().getResponse().getContentAsString();
		...
		mvc.perform(post("/v1/auth/change-password").header("Authorization", "Bearer " + updated.accessToken())
				.contentType("application/json")
	...
	private RegisterResponse register() {
		return auth.register(new RegisterRequest(EMAIL, PASSWORD));
	}
	...
	static class TestConfiguration {
		...
		@Bean
		DeliveryCollector deliveryCollector() {
			return new DeliveryCollector();
		}
	...
	static class DeliveryCollector {
		...
		public void receive(TokenDeliveryRequested event) {
			events.add(event);
		}

		TokenDeliveryRequested last() {
