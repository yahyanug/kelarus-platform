...
class SecurityConfigurationTest {
	...
	void signingKeyRejectsMissingMalformedAndShortSecretsWithoutEchoingThem() {
		for (String value : new String[] { "", "not-base64!", Base64.getEncoder().encodeToString(new byte[16]) }) {
			assertThatThrownBy(() -> configuration.jwtSigningKey(properties(value)))
					.isInstanceOf(IllegalStateException.class).hasMessageContaining("KELARUS_AUTH_JWT_SECRET");
		}
	...
	void tokenTtlsMustBePositiveAndAtLeastOneSecond() {
		assertThatThrownBy(() -> new AuthProperties(null, Duration.ZERO, Duration.ofDays(30), Duration.ofHours(24),
				Duration.ofMinutes(30))).isInstanceOf(IllegalArgumentException.class);
	...
	private AuthProperties properties(String secret) {
		return new AuthProperties(secret, Duration.ofMinutes(15), Duration.ofDays(30), Duration.ofHours(24),
