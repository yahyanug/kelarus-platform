...
public class SecurityConfiguration {
	...
	PasswordEncoder passwordEncoder() {
		// PBKDF2 supports the complete 128-character policy without BCrypt's 72-byte
		// limit.
	...
	SecretKey jwtSigningKey(AuthProperties properties) {
		...
		if (key.length < 32) {
			throw new IllegalStateException(
					"KELARUS_AUTH_JWT_SECRET must contain at least 32 random bytes, Base64-encoded.");
		}
		...
		OAuth2TokenValidator<Jwt> identity = jwt -> {
			try {
				...
				if (jwt.getExpiresAt() != null && jwt.getIssuedAt() != null
						...
						&& jwt.getExpiresAt().isAfter(jwt.getIssuedAt()) && jwt.getClaimAsString("email") != null) {
	...
	SecurityFilterChain securityFilterChain(HttpSecurity http) throws Exception {
		...
		return http.csrf(csrf -> csrf.disable()) // Bearer tokens only; no cookie/session
													// authentication.
				...
				.exceptionHandling(errors -> errors.authenticationEntryPoint((request, response, ex) -> {
					...
					response.setContentType("application/json");
					response.getWriter()
							.write("{\"code\":\"UNAUTHENTICATED\",\"message\":\"Authentication is required.\"}");
