...
public class PasswordServiceImpl implements PasswordService {
    ...
    public PasswordServiceImpl(UserRepository users, UserCredentialRepository credentials,
            PasswordResetTokenRepository resets, RefreshTokenRepository refreshTokens,
    ...
    public void forgotPassword(ForgotPasswordRequest request) {
        users.findByEmail(request.email()).filter(user -> user.getStatus() == UserStatus.ACTIVE).ifPresent(user -> {
            ...
            events.publishEvent(new TokenDeliveryRequested(TokenDeliveryRequested.Purpose.PASSWORD_RESET,
                    user.getId(), user.getEmail(), raw, expiry));
        });
    }
    ...
    public void resetPassword(ResetPasswordRequest request) {
        ...
        UserCredential credential = credentials.findByUserId(userId).orElseThrow(INVALID_PASSWORD_RESET_TOKEN::exception);
    ...
    public void changePassword(UUID authenticatedUserId, ChangePasswordRequest request) {
        ...
        UserCredential credential = credentials.findByUserId(authenticatedUserId).orElseThrow(INVALID_CREDENTIALS::exception);
